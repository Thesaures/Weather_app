...
const Home = () => {
  ...
  useEffect(() => {
    const getDetails = async () => {
      try {
        const { latitude, longitude } = await deviceLocation();

        const { detailsTask } = await weatherDetails(longitude, latitude);
        ...
        const { hourDetails } = await hourWeatherDetails(longitude, latitude);
        const { weekDetails } = await weeklyWeatherDetails(longitude, latitude);
